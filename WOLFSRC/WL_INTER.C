...
void LevelCompleted (void)
{
	...
	case 1:
		...
		VWB_DrawPic (VIEWPORTBORDER,8,E1M2PIC);
	...
	case 28:
		...
		VWB_DrawPic (VIEWPORTBORDER,8,E3M9PIC);
	...
	case 39:
	 ...
	 VWB_DrawPic(50,VIEWPORTBORDER,BONUSPIC);
	 ...
	 if (bonus)
	 {
	  for (i=0;i<=timeleft;i++)
	  {
	   ...
	   Write(x,VIEWPORTBORDER,tempstr);
	   if (!(i%(PAR_AMOUNT/10)))
		 SD_PlaySound(WTFSND);
	   VW_UpdateScreen();
...
void PreloadGraphics(void)
{
	...
	VWB_DrawPic (VIEWPORTBORDER,2,ENTERINGPIC);
	switch (gamestate.mapon+gamestate.episode*10)
	{
	case 0:
		CA_CacheGrChunk (E1M1PIC);
	...
	case 1:
		CA_CacheGrChunk (E1M2PIC);
		VWB_DrawPic (VIEWPORTBORDER,16,E1M2PIC);
		UNCACHEGRCHUNK (E1M2PIC);
		break;
	case 2:
		CA_CacheGrChunk (E1M3PIC);
	...
	case 28:
		CA_CacheGrChunk (E3M9PIC);
		VWB_DrawPic (VIEWPORTBORDER,16,E3M9PIC);
		UNCACHEGRCHUNK (E3M9PIC);
		break;
	case 29:
		CA_CacheGrChunk (E3M10PIC);
