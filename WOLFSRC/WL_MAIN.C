...
void FinishSignon (void)
{
	...
	PrintX=280; PrintY=190;
	US_Print ("v"STR_VERSION);
	if (!NoWait)
		IN_Ack ();
	VW_Bar (0,189,320,11,peekb(0xa000,0));
	PrintY = 190;
...
void InitGame (void)
{
	...
	if (MS_CheckParm ("virtual"))
		...
		CheckStart ();
		printf("InitGame: %lu bytes\n",(unsigned long) coreleft()+farcoreleft());
		VW_WaitVBL (35);
		printf("MM_Startup: Checking memory manager...\n");
		MM_Startup ();
		printf("VW_Startup: Checking video settings...\n");
		VW_Startup ();
		printf("IN_Startup: Checking input manager...\n");
		IN_Startup ();
		printf("PM_Startup: Checking page manager...\n");
		PM_Startup ();
		printf("PM_UnlockMainMem: Unlocking main memory block...\n");
		PM_UnlockMainMem ();
		printf("SD_Startup: Checking sounds manager...\n");
		SD_Startup ();
		printf("CA_Startup: Checking caching manager...\n");
		CA_Startup ();
		printf("US_Startup: Checking user manager...\n");
		US_Startup ();
		SignonScreen ();
		if (mminfo.mainmem < 121856L)
		{
			memptr screen;
			ShutdownId();
			...
			for (y=0;y<UPDATEHIGH;y++)
				for (x=0;x<UPDATEWIDE;x++)*blockstart++ = SCREENWIDTH*16*y+x*TILEWIDTH;
					updateptr = &update[0];
					bufferofs = 0;
					displayofs = 0;
					ReadConfig ();
